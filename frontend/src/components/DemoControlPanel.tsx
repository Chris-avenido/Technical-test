...
export const DemoControlPanel: React.FC<DemoControlPanelProps> = ({
  ...
  return (
    <div className="w-full bg-zinc-900 text-zinc-100 border-b border-zinc-800 px-3 sm:px-4 py-2 text-xs">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3">
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <span className="px-2 py-0.5 rounded bg-zinc-800 text-zinc-300 font-mono text-[10px] sm:text-[11px] border border-zinc-700 shrink-0">
            {t.evaluatorMode}
          </span>
          <div className="flex items-center gap-1.5 text-zinc-300 min-w-0">
            <UserCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="truncate max-w-[150px] sm:max-w-none">{userStatus?.email || 'demo@example.com'}</span>
          </div>
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3 pt-1 sm:pt-0 border-t sm:border-t-0 border-zinc-800/60">
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded bg-zinc-800 hover:bg-zinc-700 active:bg-zinc-600 text-zinc-200 border border-zinc-700 font-medium transition-colors cursor-pointer disabled:opacity-50 text-[11px] sm:text-xs shrink-0 ml-auto sm:ml-0"
          >
